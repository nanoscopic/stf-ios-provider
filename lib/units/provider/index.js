...
module.exports = function(options) {
  ...
  log.info('Receiving devEvent message from coordinator via tcp://127.0.0.1:7294')
  sock2.connect('tcp://127.0.0.1:7294');
  sock2.subscribe('devEvent');
  sock2.on( 'message', function( topic, msg ) {
    var str = msg.toString();
    if( str == "dummy" ) return;
    log.info('Received message from coordinator: %s', str )
  ...
  setInterval( function() {
    var uuids = Object.keys( connected );
    for( var i=0;i<uuids.length;i++ ) {
      var uuid = uuids[i];
      push.send ( [ wireutil.global, wireutil.envelope( new wire.DeviceHeartbeatMessage( uuid ) ) ] )
      console.log("sending heartbeat for", uuid );
    }
  }, 1000 * 20 ); // every 20 seconds ( reaper is every 30 seconds )
