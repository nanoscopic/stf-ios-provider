...
module.exports = syrup.serial()
  ...
  .define(function(options, wda, router, display, flags) {
    ...
    function TouchConsumer(config) {
      ...
      this.width = options.clickWidth
      this.height = options.clickHeight
    }
    ...
    TouchConsumer.prototype.touchMove = function(point) {
      ...
      if(Math.abs(preMovePoint.x-endX)>20||Math.abs(preMovePoint.y-endY)>20){
        bIsTouch = false
        this.bIsMove = true
    ...
    TouchConsumer.prototype.touchUp = function(point) {
      ...
      var duration = dur*50/100 // effectively /2
        
      if( this.bIsMove ) {
        if( Math.abs( endX - startX ) < 10 && Math.abs( endY - startY ) < 10 ) {
          if( duration >= 500 ) wda.clickHold(startX,startY,duration/500);
          else wda.click(startX,startY)
        }
        else {
          if (duration>2000)
            duration = 2000
