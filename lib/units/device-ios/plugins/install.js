...
module.exports = syrup.serial()
  ...
  .define(function(options, router, push, wdaCommands, identity) {
      ...
      pushApp()
        .then(function() {
          if (message.launch) {
            ...
            wdaCommands.launchApp(bundleId)
            // Progress 90%
    ...
    router.on(wire.UninstallMessage, function(channel, message) {
      ...
      if (identity.platform != 'iOS') {
        log.info("Not responding to non-iOS app uninstall message")
        return
      }

      iosDeploy().uninstall(options.serial, message.packageName)
        .then(function() {
          push.send([
            channel
          , reply.okay('success')
